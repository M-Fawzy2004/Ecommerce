...
class _SignupPageBodyState extends State<SignupPageBody> {
  ...
  void _handleSignup() {
    if (!_isFullNameValid || !_isEmailValid || !_isPasswordValid || !_isConfirmValid) {
      AppToast.error(context, message: 'auth.full_name_error'.tr()); // Using generic message for generic error click
      return;
    }
    ...
    return BlocListener<AuthCubit, AuthState>(
      listener: (context, state) {
        if (state is AuthError) {
          ...
          AppToast.error(context, message: 'auth.account_exists_unverified'.tr());
          context.push(AppRouter.verification, extra: state.email);
        }
      },
