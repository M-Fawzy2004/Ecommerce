// ignore_for_file: prefer_single_quotes
class Assets {
  Assets._();
  
  /// Assets for imagesPngElectronicsBanner
  /// assets/images/png/electronics_banner.png
  static const String imagesPngElectronicsBanner = "assets/images/png/electronics_banner.png";

  /// Assets for imagesPngFashionBanner
  /// assets/images/png/fashion_banner.png
  static const String imagesPngFashionBanner = "assets/images/png/fashion_banner.png";

  /// Assets for imagesPngHeadphones
  /// assets/images/png/headphones.png
  static const String imagesPngHeadphones = "assets/images/png/headphones.png";

  /// Assets for imagesPngIphone
  /// assets/images/png/iphone.png
  static const String imagesPngIphone = "assets/images/png/iphone.png";

  /// Assets for imagesPngMacbook
  /// assets/images/png/macbook.png
  static const String imagesPngMacbook = "assets/images/png/macbook.png";

  /// Assets for imagesPngSaleBanner
  /// assets/images/png/sale_banner.png
  static const String imagesPngSaleBanner = "assets/images/png/sale_banner.png";

  /// Assets for imagesPngTShirt
  /// assets/images/png/t_shirt.png
  static const String imagesPngTShirt = "assets/images/png/t_shirt.png";

  /// Assets for imagesPngWelcomePage
  /// assets/images/png/welcome_page.png
  static const String imagesPngWelcomePage = "assets/images/png/welcome_page.png";

  /// Assets for imagesSvgBag
  /// assets/images/svg/bag.svg
  static const String imagesSvgBag = "assets/images/svg/bag.svg";

  /// Assets for imagesSvgCall
  /// assets/images/svg/call.svg
  static const String imagesSvgCall = "assets/images/svg/call.svg";

  /// Assets for imagesSvgCart
  /// assets/images/svg/cart.svg
  static const String imagesSvgCart = "assets/images/svg/cart.svg";

  /// Assets for imagesSvgCartAdd
  /// assets/images/svg/cart_add.svg
  static const String imagesSvgCartAdd = "assets/images/svg/cart_add.svg";

  /// Assets for imagesSvgCartDownload
  /// assets/images/svg/cart_download.svg
  static const String imagesSvgCartDownload = "assets/images/svg/cart_download.svg";

  /// Assets for imagesSvgCartUpload
  /// assets/images/svg/cart_upload.svg
  static const String imagesSvgCartUpload = "assets/images/svg/cart_upload.svg";

  /// Assets for imagesSvgChart
  /// assets/images/svg/chart.svg
  static const String imagesSvgChart = "assets/images/svg/chart.svg";

  /// Assets for imagesSvgDocument
  /// assets/images/svg/document.svg
  static const String imagesSvgDocument = "assets/images/svg/document.svg";

  /// Assets for imagesSvgDownload
  /// assets/images/svg/download.svg
  static const String imagesSvgDownload = "assets/images/svg/download.svg";

  /// Assets for imagesSvgEdit
  /// assets/images/svg/edit.svg
  static const String imagesSvgEdit = "assets/images/svg/edit.svg";

  /// Assets for imagesSvgEmail
  /// assets/images/svg/email.svg
  static const String imagesSvgEmail = "assets/images/svg/email.svg";

  /// Assets for imagesSvgEye
  /// assets/images/svg/eye.svg
  static const String imagesSvgEye = "assets/images/svg/eye.svg";

  /// Assets for imagesSvgEyeClosed
  /// assets/images/svg/eye_closed.svg
  static const String imagesSvgEyeClosed = "assets/images/svg/eye_closed.svg";

  /// Assets for imagesSvgFilter
  /// assets/images/svg/filter.svg
  static const String imagesSvgFilter = "assets/images/svg/filter.svg";

  /// Assets for imagesSvgGift
  /// assets/images/svg/gift.svg
  static const String imagesSvgGift = "assets/images/svg/gift.svg";

  /// Assets for imagesSvgHeart
  /// assets/images/svg/heart.svg
  static const String imagesSvgHeart = "assets/images/svg/heart.svg";

  /// Assets for imagesSvgHome
  /// assets/images/svg/home.svg
  static const String imagesSvgHome = "assets/images/svg/home.svg";

  /// Assets for imagesSvgLeftArrow
  /// assets/images/svg/left_arrow.svg
  static const String imagesSvgLeftArrow = "assets/images/svg/left_arrow.svg";

  /// Assets for imagesSvgLocation
  /// assets/images/svg/location.svg
  static const String imagesSvgLocation = "assets/images/svg/location.svg";

  /// Assets for imagesSvgLock
  /// assets/images/svg/lock.svg
  static const String imagesSvgLock = "assets/images/svg/lock.svg";

  /// Assets for imagesSvgLockOpen
  /// assets/images/svg/lock_open.svg
  static const String imagesSvgLockOpen = "assets/images/svg/lock_open.svg";

  /// Assets for imagesSvgMenu
  /// assets/images/svg/menu.svg
  static const String imagesSvgMenu = "assets/images/svg/menu.svg";

  /// Assets for imagesSvgMobile
  /// assets/images/svg/mobile.svg
  static const String imagesSvgMobile = "assets/images/svg/mobile.svg";

  /// Assets for imagesSvgMoney
  /// assets/images/svg/money.svg
  static const String imagesSvgMoney = "assets/images/svg/money.svg";

  /// Assets for imagesSvgMoneyDollar
  /// assets/images/svg/money_dollar.svg
  static const String imagesSvgMoneyDollar = "assets/images/svg/money_dollar.svg";

  /// Assets for imagesSvgOffer
  /// assets/images/svg/offer.svg
  static const String imagesSvgOffer = "assets/images/svg/offer.svg";

  /// Assets for imagesSvgPackage
  /// assets/images/svg/package.svg
  static const String imagesSvgPackage = "assets/images/svg/package.svg";

  /// Assets for imagesSvgReload
  /// assets/images/svg/reload.svg
  static const String imagesSvgReload = "assets/images/svg/reload.svg";

  /// Assets for imagesSvgRightArrow
  /// assets/images/svg/right_arrow.svg
  static const String imagesSvgRightArrow = "assets/images/svg/right_arrow.svg";

  /// Assets for imagesSvgSearch
  /// assets/images/svg/search.svg
  static const String imagesSvgSearch = "assets/images/svg/search.svg";

  /// Assets for imagesSvgShield
  /// assets/images/svg/shield.svg
  static const String imagesSvgShield = "assets/images/svg/shield.svg";

  /// Assets for imagesSvgShop
  /// assets/images/svg/shop.svg
  static const String imagesSvgShop = "assets/images/svg/shop.svg";

  /// Assets for imagesSvgStar
  /// assets/images/svg/star.svg
  static const String imagesSvgStar = "assets/images/svg/star.svg";

  /// Assets for imagesSvgTruck
  /// assets/images/svg/truck.svg
  static const String imagesSvgTruck = "assets/images/svg/truck.svg";

  /// Assets for imagesSvgUpload
  /// assets/images/svg/upload.svg
  static const String imagesSvgUpload = "assets/images/svg/upload.svg";

  /// Assets for imagesSvgUser
  /// assets/images/svg/user.svg
  static const String imagesSvgUser = "assets/images/svg/user.svg";
}

